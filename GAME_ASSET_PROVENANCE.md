...
版本 0.3.2 build 6。工程打包的 55 张位图来自《星露谷物语》素材及以下项目镜像；应用图标由游戏图像进行最近邻缩放，不使用图像生成模型。普通勾选、箭头、加减号等仍是文字或系统控件。素材接入参考：
...
用户提供的 [stardewvalley.site 画布](https://stardewvalley.site/zh/canvas/) 在本次尝试中连接超时，未从该站取得或引用素材。实际新增素材来自已核对的 GitHub 固定提交。
...
本包未取得可靠的动物原始图集与肤色调色板，相关 UI 明确显示无预览；仍保留名称、编号与可编辑字段。发型和饰品分别显示真实图层，未合成完整人物。地图是存档实体坐标视图，不是游戏地图截图，尚未渲染完整地形、各生长阶段作物与所有升级建筑。
